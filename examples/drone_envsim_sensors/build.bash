...
OUT="$HERE/lidar_envsim_demo"
g++ -std=c++20 -O2 -Wall -Wextra \
    -I "$REPO/include" -I "$RUNTIME_INC" -isystem "$MJ_INC" \
    "$HERE/lidar_envsim_demo.cpp" \
...
if [ ! -f "$REGISTRY_DIR/sensor_msgs/pdu_cpptype_conv_LaserScan.hpp" ]; then
    echo "WARN: PDU registry types not found under $REGISTRY_DIR -- skipping lidar_laserscan_pdu (set REGISTRY_DIR)"
    exit 0
fi
OUT_PDU="$HERE/lidar_laserscan_pdu"
g++ -std=c++20 -O2 -Wall -Wextra \
    -I "$REPO/include" -I "$RUNTIME_INC" -isystem "$MJ_INC" -isystem "$REGISTRY_DIR" \
    "$HERE/lidar_laserscan_pdu.cpp" \
    "$REPO/src/sensors/backend/mujoco_ray_caster.cpp" \
    "$REPO/src/sensors/lidar/lidar_scan_sensor.cpp" \
    -L "$MJ_LIB" -lmujoco -Wl,-rpath,"$MJ_LIB" \
    -o "$OUT_PDU"
...
OUT_PDU3D="$HERE/lidar3d_pointcloud_pdu"
g++ -std=c++20 -O2 -Wall -Wextra \
    -I "$REPO/include" -I "$RUNTIME_INC" -isystem "$MJ_INC" -isystem "$REGISTRY_DIR" \
    "$HERE/lidar3d_pointcloud_pdu.cpp" \
    "$REPO/src/sensors/backend/mujoco_ray_caster.cpp" \
...
g++ -std=c++20 -O2 -Wall -Wextra \
    ...
    -o "$OUT_SW"
echo "built: $OUT_SW"
echo "verify: python3 verify_pdu.py ; verify_pdu_3d.py ; verify_runtime.py ; verify_drone_manifest.py ; verify_sensing_mode_switch.py"
