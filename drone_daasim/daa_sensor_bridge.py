...
path radar_receiver.py / takeoff.py use), then loops:
  ...
  sense the env.xml obstacle world from that pose with the REAL A-2
        mujoco-sensor 3D LiDAR (examples/.../lidar3d_pointcloud_pdu)
...
Usage: python daa_sensor_bridge.py <env.xml> <lidar3d_a2_pdu_bin> [hz=5]
"""
import os
import sys
import time
    ...
    try:
        while True:
            ...
            if raw and len(raw) > 0:
                ...
                if tw is not None:
                    reads += 1
                    # rebuild a clean pos.bin in the format lidar3d_pointcloud_pdu expects
                    t2 = Twist()
